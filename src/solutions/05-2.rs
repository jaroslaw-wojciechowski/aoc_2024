...
fn parse_input(file_name: &str, order_map: HashMap<i32, Vec<i32>>) {
    ...
    if let Ok(lines) = read_lines(file_name) {
        for line in lines.flatten() {
            let mut vector: Vec<i32> = line.split(",").map(|val| val.parse().unwrap()).collect();
            if !validate(&mut vector.clone(), &order_map) {
                vector = aoc_sort(vector, &order_map);
...
fn aoc_sort(vector: Vec<i32>, order_map: &HashMap<i32, Vec<i32>>) -> Vec<i32> {
    let mut tmp_vector = vector.clone();

    tmp_vector.sort_by(|a, b| {
        if !order_map.contains_key(&a) {
            Ordering::Greater
        } else if !order_map.contains_key(&b) {
            Ordering::Less
        } else if order_map.get(a).unwrap().contains(b) {
            Ordering::Less
        } else if order_map.get(b).unwrap().contains(a) {
            Ordering::Greater
        } else {
            Ordering::Equal
        }
