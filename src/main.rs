...
fn process_input(in_map: &HashMap<String, i64>) -> HashMap<String, i64> {
    ...
    for (key, val) in map.iter() {
        if key.eq("0") {
            let count: i64 = *out_map.get(&String::from("1")).unwrap_or(&0);
            out_map.insert(String::from("1"), val.clone() + count);
        } else if key.eq("1") {
            let count: i64 = *out_map.get(&String::from("2024")).unwrap_or(&0);
            out_map.insert(String::from("2024"), val.clone() + count);
        } else if key.eq("20") {
            let count: i64 = *out_map.get(&String::from("2")).unwrap_or(&0);
            out_map.insert(String::from("2"), val.clone() + count);
            let count: i64 = *out_map.get(&String::from("0")).unwrap_or(&0);
            out_map.insert(String::from("0"), val.clone() + count);
        } else if key.eq("24") {
            let count: i64 = *out_map.get(&String::from("2")).unwrap_or(&0);
            out_map.insert(String::from("2"), val.clone() + count);
            let count: i64 = *out_map.get(&String::from("4")).unwrap_or(&0);
            out_map.insert(String::from("4"), val.clone() + count);
        } else if key.eq("2024") {
            let count: i64 = *out_map.get(&String::from("20")).unwrap_or(&0);
            out_map.insert(String::from("20"), val.clone() + count);
            let count: i64 = *out_map.get(&String::from("24")).unwrap_or(&0);
            out_map.insert(String::from("24"), val.clone() + count);
        } else if key.len() % 2 == 0 {
            let (l, r) = split_stone(key.clone());
            let count: i64 = *out_map.get(&l).unwrap_or(&0);
            out_map.insert(l, val.clone() + count);
            let count: i64 = *out_map.get(&r).unwrap_or(&0);
            out_map.insert(r, val.clone() + count);
        } else if key.len() % 2 != 0 {
            let mul_op = mul_stone(key.clone());
            let count: i64 = *out_map.get(&mul_op).unwrap_or(&0);
